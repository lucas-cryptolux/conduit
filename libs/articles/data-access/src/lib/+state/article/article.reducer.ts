  ...
  reducer: createReducer(
    ...
    on(articleActions.followSuccess, articleActions.unfollowSuccess, (state, action) => {
      const data: Article = {
        ...state.data,
        author:
          state.data.author.username === action.profile.username
            ? { ...state.data.author, following: action.profile.following }
            : state.data.author,
        coauthors: state.data.coauthors.map((coauthor) =>
          coauthor.username === action.profile.username
            ? { ...coauthor, following: action.profile.following }
            : coauthor,
        ),
      };
      return { ...state, data };
