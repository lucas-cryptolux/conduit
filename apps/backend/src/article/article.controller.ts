...
export class ArticleController {
  ...
  @ApiOperation({ summary: 'Start editing an article' })
  @ApiResponse({ status: 200, description: 'Successfully started editing the article.' })
  @ApiResponse({ status: 403, description: 'Forbidden. The article is already being edited by another user.' })
  @ApiResponse({ status: 404, description: 'Not found. The article does not exist.' })
  @Post(':slug/edit')
  async startEditing(@User('id') userId: number, @Param('slug') slug: string) {
    return this.articleService.startEditing(userId, slug);
  }
